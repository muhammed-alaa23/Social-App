...
import { addComments } from '../services/commentAServices';
import { set } from 'zod';
...
export default function Post({ posts, commentsLimit , callBack, displayPosts }) {
  ...
  const { userData } = useContext(AuthContext);
    ...
    const [moreComments, setmoreComments] = useState(5);
      ...
      const [commentSubmmiting, setcommentSubmmiting] = useState(false);
      ...
      async function handleDeletePost(onClose) {
        ...
        const response = await deletePostApi(posts.id);
      ...
      async function handleAddComment() {
        setcommentSubmmiting(true)
        if (commentContent.trim()) {
          const response = await addComments({ commentContent, postId: posts.id });
          console.log(response);
          if (response.message === "success") {
            setCommentContent('');
            callBack();
  ...
  return (
    ...
    <div key={posts.id} className="shadow-2xl max-w-screen-md bg-white mt-6 rounded-2xl p-4 ">
    <div className="flex items-center justify-between">
      <div className="gap-3.5	flex items-center ">
        <img src={posts.user.photo} className="object-cover	rounded-full w-10 h-10" />
        <div className="flex flex-col">
          <b className="mb-2 capitalize">{posts.user.name}</b>
          <time dateTime="06-08-21" className="text-gray-400 text-xs">{posts.createdAt}</time>
        </div>
      </div>
      <div className=" flex	items-center justify-center cursor-pointer ">
      {posts.user._id == userData._id && <Dropdown>
        <DropdownTrigger>
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="34px" fill="#92929D" className='outline-none w-fit rotate-90 '>
    ...
    <div className="whitespace-pre-wrap mt-7">{posts.body}</div>
    <div className="mt-5 flex gap-2	 justify-center pb-4 flex-wrap	">
      {posts.image && <img src={posts.image} className=" rounded-2xl w-1/3 object-cover h-96 flex-auto" alt="photo" />}
    </div>
    <div className=" h-16 border-t border-divider mt-3 border-b  flex items-center justify-around	">
      <div onClick={() => {navigate ("/post-details/"+posts.id)}} className="flex items-center	gap-3	bg-gray-100 rounded-full px-3 py-2 cursor-pointer">
      ...
      <div  className="flex items-center gap-3 cursor-pointer bg-gray-100 rounded-full px-3 py-2">
      ...
      <div className="flex items-center	gap-3 bg-gray-100 rounded-full px-3 py-2 cursor-pointer">
    ...
    {posts.comments.slice(0, commentsLimit ?? moreComments).map((comment) => (
      <Comment key={comment._id} comment={comment} posts={posts} callBack={callBack} displayPosts={displayPosts} />
    ))}
    <div className=' mb-[-16px] py-4 px-8 mx-[-16px]  relative bg-gray-100'>
      <input value={commentContent} onChange={(e) => setCommentContent(e.target.value)} type="text" className='border-b-2 border-gray-300 block w-full p-2 outline-0' placeholder='Add a comment...' />
      <div className='absolute bottom-4 right-4'>
        <Button isLoading={commentSubmmiting} className='text-xs border-none' variant='ghost' color='primary' onPress={handleAddComment}>Comment</Button>
      </div>
    </div>

    {posts.comments.length > moreComments && !commentsLimit &&
      <Button className='mx-auto block mt-6 text-xs'  onPress={handleLoadMore} isLoading={isLoading}>
          ...
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                ...
                <Button color="danger" isLoading={isPostDeleting} onPress={()=>handleDeletePost(onClose)}>
                  Delet
                </Button>
              </ModalFooter>
            </>
